...
class Networking {
    ...
    private func _interfaceAddressForName (_ name: String, _ requestType: AddressRequestType) throws -> String {
        ...
        strncpy(&buffer, name, ifNameSize)
        ...
        let ioRequest: UInt32 = {
            switch requestType {
            case .ipAddress:
                // Magic number SIOCGIFADDR - see sockio.h
                return _IOWR("i", 33, UInt32(MemoryLayout<ifreq>.size))
            case .netmask:
                // Magic number SIOCGIFNETMASK
                return _IOWR("i", 37, UInt32(MemoryLayout<ifreq>.size))
            }
        }()
